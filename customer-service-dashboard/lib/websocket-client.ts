...
class WebSocketClient {
  ...
  private setupEventListeners() {
    ...
    // 监听群组操作事件
    this.socket.on('group_name_changed', (data) => {
      try {
        console.log('📝 [WS:group_name_changed] 群组名称变更:', data);
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('chatx:group_name_changed', { detail: data }));
        }
      } catch (error) {
        console.error('❌ [WS:group_name_changed] 处理群组名称变更失败:', error);
      }
    });

    this.socket.on('group_photo_changed', (data) => {
      try {
        console.log('🖼️ [WS:group_photo_changed] 群组头像变更:', data);
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('chatx:group_photo_changed', { detail: data }));
        }
      } catch (error) {
        console.error('❌ [WS:group_photo_changed] 处理群组头像变更失败:', error);
      }
    });

    this.socket.on('group_member_added', (data) => {
      try {
        console.log('➕ [WS:group_member_added] 成员加入:', data);
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('chatx:group_member_added', { detail: data }));
        }
      } catch (error) {
        console.error('❌ [WS:group_member_added] 处理成员加入失败:', error);
      }
    });

    this.socket.on('group_member_removed', (data) => {
      try {
        console.log('➖ [WS:group_member_removed] 成员移除:', data);
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('chatx:group_member_removed', { detail: data }));
        }
      } catch (error) {
        console.error('❌ [WS:group_member_removed] 处理成员移除失败:', error);
      }
    });

    this.socket.on('group_member_joined_by_link', (data) => {
      try {
        console.log('🔗 [WS:group_member_joined_by_link] 通过链接加入:', data);
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('chatx:group_member_joined_by_link', { detail: data }));
        }
      } catch (error) {
        console.error('❌ [WS:group_member_joined_by_link] 处理通过链接加入失败:', error);
      }
    });

    this.socket.on('message_pinned', (data) => {
      try {
        console.log('📌 [WS:message_pinned] 消息置顶:', data);
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('chatx:message_pinned', { detail: data }));
        }
      } catch (error) {
        console.error('❌ [WS:message_pinned] 处理消息置顶失败:', error);
      }
    });

    this.socket.on('group_action', (data) => {
      try {
        console.log('🔄 [WS:group_action] 群组操作:', data);
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('chatx:group_action', { detail: data }));
        }
      } catch (error) {
        console.error('❌ [WS:group_action] 处理群组操作失败:', error);
      }
    });

    this.socket.on('group_updated', (data) => {
      try {
        console.log('🔄 [WS:group_updated] 群组更新:', data);
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('chatx:group_updated', { detail: data }));
        }
      } catch (error) {
        console.error('❌ [WS:group_updated] 处理群组更新失败:', error);
      }
    });
  }
